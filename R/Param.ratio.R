...
if(is.null(addargs$var.equal))
 ...
 if(addargs$var.equal)
  ...
  {METHOD<-"Ratio of means assuming Normal distribution, allowing unequal variances"}
   ...
   temp <- do.call(what="ratiottest", args=addargs)
...
ratiottest <- function (x, y, alternative = "two.sided", rho = 1, var.equal = FALSE, conf.level = 0.95, iterativeCI=FALSE, ul=1e10, ll=-1e10, ...) 
{
